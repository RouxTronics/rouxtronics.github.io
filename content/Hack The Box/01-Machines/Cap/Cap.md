...
Cap is an easy Linux machine that exposes a Flask/Gunicorn web dashboard running network capture utilities.

The application generates PCAP files tied to a numeric ID in the URL — but performs no ownership check, making every capture accessible to any authenticated session. Iterating to `/data/0` reveals an older capture containing FTP credentials transmitted in plaintext. 

Those credentials reuse across SSH, granting an initial foothold as `nathan`. 

Local enumeration reveals that `/usr/bin/python3.8` carries the `cap_setuid+ep` Linux capability, allowing any process spawned by it to arbitrarily change its UID — a one-liner escalation to root.

**Core attack chain:** Predictable numeric IDs with no authorisation check → plaintext FTP creds in PCAP → credential reuse on SSH → Linux capability abuse (`cap_setuid`) → root.
...
- **IDOR exploitation** — identifying and iterating over insecure direct object references in web applications to access resources belonging to other users
- **Linux capabilities abuse** — understanding how `cap_setuid` differs from SUID and why it's exploitable with a single Python one-liner
- **Credential reuse assessment** — always testing recovered credentials across every exposed service before moving on
...
No lateral movement required. Direct path from `nathan` to root via capability abuse.
...
> [!success] Owned!
> **User:** `f63e490a8a3fd6f7032ac8e7f0b89e1c`
> **Root:** `3540f38a944802969e78336b1745a481`
...
> 👉 **Sign up, complete the _Intro to Academy_ module, and start earning cubes.**  
> 🚀 I earn cubes as you progress — win‑win for both of us!
> 
> 🔗 **Join here:**  [https://referral.hackthebox.com/mzC9F4k](https://referral.hackthebox.com/mzC9F4k)
{: .prompt-info }
